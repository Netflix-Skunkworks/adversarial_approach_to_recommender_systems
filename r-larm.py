...
@tf.function
def adversary_loss(U, U_adv, train, B):
    '''
    Args:
        U: learner model
        U_adv: adversarial model
    ...
    target_loss = tf.reduce_sum(tf.multiply(loss_matrix, weight_matrix)) - B*tf.nn.l2_loss(U_adv)
    return target_loss

@tf.function
def adv_loss_grad(U, U_adv, target, B):
    '''
    Args:
        U: learner model
        U_adv: adversarial model
    ...
    with tf.GradientTape(watch_accessed_variables=False) as t:
        t.watch([U_adv])
        l= -adversary_loss(U, U_adv, target, B)  
    [gU_adv] = t.gradient(l, [U_adv])
    return (gU_adv,l)

@tf.function
def learner_loss(U, U_adv, train, C):
    '''
    Args:
        U: learner model
        U_adv: adversarial model
...
@tf.function
def learner_loss_grad(U, U_adv, target,  C):
    '''
    Args:
        U: learner model
        U_adv: adversarial model
    ...
    with tf.GradientTape(watch_accessed_variables=False) as t:
        t.watch([U])
        l= learner_loss(U, U_adv,  target,  C)
    gU = t.gradient(l, U)
    return (gU,l)
...
def adv_loop(train, train_params, test_tr, test_te, valid_tr, valid_te):
    ...
    for it in range(0,train_params.num_iter):
        ...
        for i in range(0, int(train.shape[0]/train_params.batch_size)):
            t_temp = target_index_ds.next().numpy()[0]
            iter_train = sparse_sample(train_params.batch_size, train, target_index_ds.next().numpy()[0])
            (pu, l1) = learner_loss_grad(model.U, adv_model.U, iter_train, train_params.C)
            optimizer.apply_gradients(zip([pu], [model.U]))
            tot_loss = tot_loss + l1
            if it >= 1:
                (pu, l1) = adv_loss_grad(model.U, adv_model.U,  iter_train, train_params.B)
...
if __name__ == '__main__':
    ...
    lr=5e-5 
    batch_size=1024
    load_train_and_save(C, B, lr, base_input_path, base_output_path,  batch_size=batch_size,  num_iter=100) 
